...
public class DataInitializer implements CommandLineRunner {
    ...
    private void seedUsers() {
        ...
        List<UserEntity> users = Arrays.asList(
                ...
                UserEntity.builder()
                        ...
                        .certifications(Arrays.asList("USDA Organic", "India Organic NPOP", "FairWild Certified"))
                ...
                UserEntity.builder()
                        ...
                        .certifications(Arrays.asList("GMP Certified", "ISO 22000:2018", "HACCP"))
                ...
                UserEntity.builder()
                        ...
                        .certifications(Arrays.asList("ISO/IEC 17025 Accredited", "NABL Recognized", "US FDA Audited"))
                ...
                UserEntity.builder()
                        ...
                        .certifications(Arrays.asList("GDP Compliant", "ISO 9001:2015"))
                ...
                UserEntity.builder()
                        ...
                        .certifications(Arrays.asList("Organic Retailers Guild Member", "Soil Association Certified"))
    ...
    private void seedProducts() {
        ...
        p1.setProcessingDetails(ProcessingDetailsEntity.builder()
                ...
                .facilityLocation("PhytoExtracts Facility B, Bangalore Biotech Park, India")
                ...
                .notes("Cryogenic milling at -40°C preserved thermo-sensitive withanolide glycosides without thermal degradation.")
        ...
        LabReportEntity lab1 = LabReportEntity.builder()
                .id("LAB-2024-8901")
                .labId("USR-LAB-01")
                .labName("Dr. Ananya Sharma (Eurofins AgriBio)")
                ...
                .notes("Sample fully complies with USP <561> Articles of Botanical Origin. No synthetic adulterants detected.")
        ...
        p1.setShipmentDetails(ShipmentEntity.builder()
                ...
                .distributorName("Klaus Lindner (TransGlobal Logistics)")
                .sourceLocation("PhytoExtracts Hub, Bangalore, India")
                .destinationLocation("Pure Botanical Apothecary, London, UK")
                .vehicleNumber("TG-AIR-4482 / UK-TRK-771")
                .transportType(TransportType.REFRIGERATED_TRUCK)
                .temperatureRange("15°C – 22°C (Monitored)")
                .dispatchDate(LocalDate.of(2024, 7, 8))
                .expectedDeliveryDate(LocalDate.of(2024, 7, 14))
                .actualDeliveryDate(LocalDate.of(2024, 7, 13))
                .status("DELIVERED")
                .trackingNumber("TRK-GB-2024-998812")
        ...
        p1.setRetailDetails(RetailDetailsEntity.builder()
                ...
                .retailerName("Sophia Laurent (Pure Botanical Apothecary)")
                .storeLocation("Covent Garden Store, 14 Floral St, London WC2E 9DH")
                .receivedDate(LocalDate.of(2024, 7, 15))
                .shelfBatchId("SHELF-LON-2024-089A")
                .unitPrice(38.50)
        ...
        p1.getTimeline().addAll(Arrays.asList(
                ...
                TimelineEventEntity.builder().id("EVT-002").title("Bio-Processing & Cryogenic Milling").stage("PROCESSING").timestamp(LocalDateTime.of(2024, 6, 22, 14, 0)).actorName("Marcus Thorne").actorRole(UserRole.PROCESSOR).location("Bangalore, India").description("Cryogenic milling at -40°C completed.").txHash("0x8f23b1...").status("COMPLETED").product(p1).build(),
                TimelineEventEntity.builder().id("EVT-003").title("Laboratory Purity & Contaminant Analysis").stage("LAB_TESTING").timestamp(LocalDateTime.of(2024, 7, 2, 11, 30)).actorName("Dr. Ananya Sharma").actorRole(UserRole.LABORATORY).location("Eurofins AgriBio Lab").description("Passed all HPLC & heavy metal assays.").txHash("0x4a7c8e...").status("COMPLETED").product(p1).build(),
                TimelineEventEntity.builder().id("EVT-004").title("Cold-Chain Transit Dispatched & Delivered").stage("LOGISTICS_TRANSIT").timestamp(LocalDateTime.of(2024, 7, 13, 16, 20)).actorName("Klaus Lindner").actorRole(UserRole.DISTRIBUTOR).location("London, UK").description("Delivered safely to London retail hub.").txHash("0x2c5e8b...").status("COMPLETED").product(p1).build(),
                TimelineEventEntity.builder().id("EVT-005").title("Retail Stocking & Consumer QR Active").stage("RETAIL_ARRIVAL").timestamp(LocalDateTime.of(2024, 7, 15, 9, 30)).actorName("Sophia Laurent").actorRole(UserRole.RETAILER).location("London Covent Garden").description("Active on shelf. Authenticity QR live.").txHash("0x1a3d5f...").status("COMPLETED").product(p1).build()
        ));

        // Blockchain transactions for Product 1
        p1.getBlockchainTransactions().addAll(Arrays.asList(
                BlockchainTransactionEntity.builder().txId("0x1a3d5f7b90123456789abcdef0123456789abcdef0123456789abcdef0123456").blockNumber(10745L).timestamp(LocalDateTime.of(2024, 7, 15, 9, 30)).stage("RETAIL_ARRIVAL").action("CONFIRM_RETAIL_RECEIPT").actor("Sophia Laurent").actorRole(UserRole.RETAILER).payloadHash("sha256:b8c9d0...").endorsingPeers(Arrays.asList("peer0.consortium", "peer1.retail")).channelName("botanical-supply-channel").chaincode("BotanicalTraceability v1.0").product(p1).build(),
                BlockchainTransactionEntity.builder().txId("0x2c5e8b0d123456789abcdef0123456789abcdef0123456789abcdef012345678").blockNumber(10744L).timestamp(LocalDateTime.of(2024, 7, 8, 9, 0)).stage("LOGISTICS_TRANSIT").action("CREATE_SHIPMENT").actor("Klaus Lindner").actorRole(UserRole.DISTRIBUTOR).payloadHash("sha256:a7b8c9...").endorsingPeers(Arrays.asList("peer0.consortium", "peer1.logistics")).channelName("botanical-supply-channel").chaincode("BotanicalTraceability v1.0").product(p1).build(),
