...
export const Sidebar: React.FC = () => {
  ...
  return (
    <>
      {/* ── Mobile & Tablet Sticky Horizontal Sub-Navigation (< lg) ── */}
      <div className="lg:hidden w-full bg-white/95 backdrop-blur-md border-b border-slate-200 sticky top-[100px] z-30 shadow-2xs">
          ...
          {navLinks.map(link => {
            ...
            return (
              <NavLink
                ...
                to={link.to}
                className={({ isActive }) =>
                  `flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold transition-all shrink-0 whitespace-nowrap ${
                    isActive
      ...
      {/* ── Desktop Fixed Docked Sidebar (>= lg) ── */}
      <aside className="hidden lg:flex w-64 bg-white text-slate-800 flex-col justify-between shrink-0 border-r border-slate-200/90 fixed top-[96px] bottom-0 left-0 overflow-y-auto shadow-xs z-30">
        {/* Upper Section */}
